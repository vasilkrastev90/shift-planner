...
class CalendarPanel extends JPanel
{
  ...
  public CalendarPanel(int year, int month)
  {
	  ...
	  for(int i = 1; i != dayofweek; i++) add(new JLabel("",JLabel.CENTER));
	  while(day <= gc.getActualMaximum(Calendar.DAY_OF_MONTH)) {
	  ...
	  for(int i = gc.get(Calendar.DAY_OF_WEEK); i <= 7; i++) add(new JLabel("",JLabel.CENTER));
  	}
}